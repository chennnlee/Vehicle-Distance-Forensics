...
body('資料集未提供雷達欄位之定義文件,故先以資料自證:相對速度欄之最小值為 −29.65 m/s,'
     '而同一時刻之 CAN 車速為 29.2 m/s——靜止之路側物體回傳「負的自車速」,'
     '此一恆等關係即確立欄位語意。另有一項實作陷阱須記載:該雷達使用十六個可重複配置之 '
     'CAN 軌位,且會將同一物體同時登錄於兩個軌位,故軌位編號並非物體編號;'
     '若依軌位追蹤,一條長達三十秒之前車軌跡會被切成一百四十二段(最長僅十三幀),'
     '須改以距離與橫向之連續性重新串接(串接後最長段為一百一十五幀)。')
...
body('素材為同一車輛(dongle b0c9d2329ad1606b)之三個路段,各 60 秒 / 1,200 幀 / 20 Hz,'
     '挑選條件為「具備一條夠長且距離跨幅夠大之前車雷達軌跡」:seg21 之前車自 7.4 m '
     '拉開至 47.2 m(跨 6.4 倍,為尺度誤差最具鑑別力之條件)、seg6 為 25~34 m、'
     'seg10 為 37~62 m。管線以 4.6 節相同之參數執行,距離修正係數一律取預設值 1.0,'
     '故所得結果即為既有封存行為。共取得 908 個配對影格。')
...
out = f'{OUT_DIR}/成果報告草稿_20260903.docx'
doc.save(out)
print('saved:', out)
