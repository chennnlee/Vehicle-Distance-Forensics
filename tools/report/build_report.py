...
body('🔴 本節指出一項應予修正之缺陷:系統於遠場已拒發速度,但「距離」仍照常輸出於'
     '影片標註與資料檔中,且不附任何警語,而該處距離之中位偏差達 20%、最壞達 72%'
     '(seg21 遠場:本系統 38 m,雷達 66 m)。對鑑識用途而言,將一個未經標示之'
     '大幅偏差量印為量測值具有實質風險,建議遠場距離比照速度予以標註。'
