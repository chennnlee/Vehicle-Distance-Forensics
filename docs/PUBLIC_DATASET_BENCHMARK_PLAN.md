...
## 一之三、comma2k19 實跑結果（2026-08-10）——第 3 點已有可交付數字
...
- 素材:HF `commaai/comma2k19` 的 `raw_data/Chunk_1.zip`（8.7 GB,MIT）。188 段 60 s / 21 條路線 /
  同一台車。依「中位速 ≥ 70 km/h」取高速公路段、每路線最多 2 段 → **24 段 / 24 分鐘 / 28,703 幀**,
...
- **端到端準確度（cycle = 14.63 m,完全不用真值）**:覆蓋 84.3%;
  對 CAN MAE 11.19（CI 10.85–11.54,bias +9.95,p50 2.04,±3 內 70.9%）、
  對位姿 MAE 10.54（bias +8.96,**p50 1.20**,±3 內 78.4%);逐段 MAE 中位 4.66。
- **p50 1.2 而 p90 59 → 誤差由少數倍速誤鎖主導**。逐幀分群:87.7% 在漆線基頻、
  **10.5% 恰好在一半**、2× 0.5%。只看基頻幀（n=21,059）:對位姿 **MAE 1.43、bias +0.67、
  ±3 內 89.4%、中位相對誤差 1.01%**;對 CAN MAE 2.09。（此列用真值分群,非公正指標,
  性質同 KITTI 那次的診斷。）
- **🔴 新失效邊界:路釘造成的半週期**。加州高速公路在兩條漆線中間放反光路釘,
  實測短週期 7.37–7.44 m ≈ 24 ft = 漆線週期的一半 —— **兩個週期都是路面上真實存在的**,
  亮度自相關無從分辨。已用實驗排除三個替代解釋:搜尋帶寬（±60 px 重跑幾乎沒變）、
  自動挑點跨線（與 MAE 無相關）、取樣率（10.4 幀/週期,高於 ≥8 判準）。
  誤鎖是**速度閘控**的:60–70 km/h 佔 74.1%,120–140 km/h 0.0%,因為碼表的 lag 下限 0.25 s
  讓 7.32 m 週期只在 **v ≤ 7.32/0.25 = 105.4 km/h** 時落在搜尋範圍內。
  → 可寫成與 KITTI 那條並列的條件:**路面若有「漆線週期整數分之一」的次級週期,
  碼表在 `v ≤ cycle_sub / lag_min` 的速度帶會系統性倍速誤鎖。**
...
端到端 MAE 10.54（對 GNSS-INS 位姿真值)、量測鏈 MAE 1.43,誤差分位數與 95% CI 都有。
